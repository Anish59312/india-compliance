...
- **Write ZERO comments** — no one-liners, banners, TODOs, or docstrings. Leave the existing comments alone. The only exception: a docstring on a `@frappe.whitelist()` method, and only when I ask for it.
